...
class TwitchAPI:
	...
	def check_follow(self, user: User):
		request = requests.get(f"https://api.twitch.tv/helix/users/follows?from_id={user.user_id}&to_id=28579002", headers={"Authorization": f"Bearer {user.access_token}", "Client-Id": self.client_id})
		if request.status_code != 200:
			new_access_token, new_refresh_token = self.refresh_access_token(user.refresh_token)
			user.update_access_token(new_access_token, new_refresh_token)
